...
public class Position : ResponseBase
{
    ...
    internal static void Handle(JToken token, Action<string> logWarning, ConfigurationState config, Subject<Position[]> subject)
    {
        var data = token[2];
        if (data.Type != JTokenType.Array)
        {
            logWarning("Positions - Invalid message format, third param not array");
            return;
        }
        ...
        {
            SetGlobalData(position, logWarning, config, token, 2, true);
        }
    ...
    internal static void Handle(JToken token, Action<string> logWarning, ConfigurationState config, Subject<Position> subject)
    {
        ...
        if (data.Type != JTokenType.Array)
        {
            logWarning("Position - Invalid message format, third param not array");
            return;
        }
