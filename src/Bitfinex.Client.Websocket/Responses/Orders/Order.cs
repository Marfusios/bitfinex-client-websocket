...
public class Order
{
    ...
    internal static void Handle(JToken token, Action<string> logWarning, Subject<Order[]> subject)
    {
        var data = token[2];
        if (data.Type != JTokenType.Array)
        {
            logWarning("Orders - Invalid message format, third param not array");
            return;
        }
    ...
    internal static void Handle(JToken token, Action<string> logWarning, Subject<Order> subject)
    {
        ...
        if (data.Type != JTokenType.Array)
        {
            logWarning("Order info - Invalid message format, third param not array");
            return;
        }
