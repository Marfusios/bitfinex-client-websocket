...
namespace Bitfinex.Client.Websocket.Responses.FundingCredits;

/// <summary>
/// Info about funding credit
/// </summary>
[DebuggerDisplay("Credit: {Id} - {Symbol} - {Amount}")]
[JsonConverter(typeof(FundingCreditConverter))]
public class FundingCredit
{
    /// <summary>
    /// Offer ID
    /// </summary>
    ...
    /// <summary>
    /// The pair that the currency was used for
    /// </summary>
    ...
    internal static void Handle(JToken token, Action<string> logWarning, Subject<FundingCredit[]> subject)
    {
        var data = token[2];
        if (data.Type != JTokenType.Array)
        {
            logWarning("Funding Credits - Invalid message format, third param not array");
            return;
        }

        var parsed = data.ToObject<FundingCredit[]>();
    ...
    internal static void Handle(JToken token, Action<string> logWarning, Subject<FundingCredit> subject)
    {
        ...
        if (data.Type != JTokenType.Array)
        {
            logWarning("Funding Credit - Invalid message format, third param not array");
            return;
        }
