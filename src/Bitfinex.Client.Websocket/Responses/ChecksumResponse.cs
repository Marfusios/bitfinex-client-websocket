...
public class ChecksumResponse : ResponseBase
{
    ...
    internal static void Handle(JToken token, Action<string> logWarning, SubscribedResponse subscription, ConfigurationState config, Subject<ChecksumResponse> subject)
    {
        ...
        SetGlobalData(response, logWarning, config, token, 2);
