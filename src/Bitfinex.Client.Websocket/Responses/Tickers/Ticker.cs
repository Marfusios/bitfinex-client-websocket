...
public class Ticker : ResponseBase
{
    ...
    internal static void Handle(JToken token, Action<string> logWarning, SubscribedResponse subscription, ConfigurationState config, Subject<Ticker> subject)
    {
        var data = token[1];
        ...
        SetGlobalData(ticker, logWarning, config, token);
