...
public class PrivateTrade : ResponseBase
{
    ...
    internal static void Handle(JToken token, Action<string> logWarning, ConfigurationState config, Subject<PrivateTrade> subject, TradeType type)
    {
        var data = token[2];
        if (data.Type != JTokenType.Array)
        {
            logWarning("Private trade info - Invalid message format, third param not array");
            return;
        }

        var trade = data.ToObject<PrivateTrade>();
        trade.Type = type;
        SetGlobalData(trade, logWarning, config, token, 2, true);
