...
namespace Bitfinex.Client.Websocket.Sample.WinForms.Statistics
{
    class TradeStatsComputer
    {
        ...
        public TradeStats GetStatsFor(int minutes)
        {
            ...
            var trades = _lastTrades.Where(x => x.Mts >= timeLimit).ToArray();

            var buys = trades.Where(x => x.Amount > 0).Sum(x => x.Amount);
            var sells = trades.Where(x => x.Amount < 0).Sum(x => Math.Abs(x.Amount));
